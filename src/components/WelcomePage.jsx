    ...
    <div style={{ background: 'var(--cream)', overflowX: 'hidden' }}>
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 3, maxWidth: '660px' }}>
          ...
          {/* CTA */}
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap', animation: 'revealUp 0.7s cubic-bezier(0.22,1,0.36,1) 0.5s both' }}>
            <button onClick={onGetStarted}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'var(--ink)', color: 'var(--white)', border: 'none', borderRadius: 'var(--radius-md)', padding: '0.9rem 1.9rem', fontSize: '0.95rem', fontFamily: 'DM Sans, sans-serif', fontWeight: 600, cursor: 'pointer', transition: 'all 220ms ease', boxShadow: '0 4px 18px rgba(26,23,20,0.2)' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px) scale(1.02)'; e.currentTarget.style.boxShadow = '0 10px 28px rgba(26,23,20,0.26)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 4px 18px rgba(26,23,20,0.2)'; }}
            >
              Get started <ArrowRight size={16} />
            </button>
            <button onClick={onGetStarted}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'var(--white)', color: 'var(--ink)', border: '1px solid var(--cream-border)', borderRadius: 'var(--radius-md)', padding: '0.9rem 1.5rem', fontSize: '0.95rem', fontFamily: 'DM Sans, sans-serif', fontWeight: 500, cursor: 'pointer', transition: 'all 200ms ease' }}
