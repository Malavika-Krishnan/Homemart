...
export const LoginPage = ({ initialJoinCode, onBack }) => {
  ...
  const handleCreateAccount = (e) => {
    ...
    if (!passRegex.test(newPass.trim())) {
    ...
    if (accountType === 'admin') {
      ...
      createFamilySpace({ adminName: newName.trim(), adminEmail: newEmail.trim(), familyName: familyName.trim(), avatar: avatarDataUrl, password: newPass.trim() });
    } else {
      if (!joinCode.trim()) { setSignupError('Please enter an invite code.'); return; }
      joinFamilySpace({ code: joinCode.trim().toUpperCase(), memberName: newName.trim(), memberEmail: newEmail.trim(), avatar: avatarDataUrl, password: newPass.trim() });
    }
  };
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        {mode === 'signin' && (
          <div className="card" style={{ padding: '1.5rem' }}>
            <p style={{ fontSize: '1rem', fontWeight: 600, fontFamily: 'DM Sans, sans-serif', color: 'var(--ink)', marginBottom: '0.2rem' }}>
              Welcome back
            </p>
