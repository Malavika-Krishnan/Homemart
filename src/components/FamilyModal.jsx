...
export const FamilyModal = ({ isOpen, onClose }) => {
  const { currentUser, familySpace, removeMember, showToast } = useApp();
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-panel anim-scale-in" onClick={e => e.stopPropagation()} style={{ padding: '1.5rem' }}>
        ...
        {/* Invite code & permanent share link */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
            <div>
              <p style={{ fontSize: '0.72rem', color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.2rem' }}>Permanent Invite Code</p>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', paddingTop: '0.5rem', borderTop: '1px dashed var(--cream-border)' }}>
            <input
              type="text"
              readOnly
              value={`${window.location.origin}/?join=${familySpace.code}`}
              style={{ flex: 1, fontSize: '0.75rem', padding: '0.35rem 0.6rem', background: 'var(--white)', border: '1px solid var(--cream-border)', borderRadius: '6px' }}
            />
            <button
              onClick={() => {
                navigator.clipboard.writeText(`${window.location.origin}/?join=${familySpace.code}`);
                showToast('Permanent family invite link copied!');
              }}
          ...
          {familySpace.members.map(m => {
            ...
            return (
              <div key={m.id} style={{
                ...
                {isAdmin && !mAdmin && (
                  <button onClick={() => removeMember(m.id || m._id)} className="btn btn-ghost btn-icon btn-sm" style={{ color: 'var(--red-soft)' }} title="Remove member from family">
                    <UserX size={13} />
                  </button>
                )}
              </div>
